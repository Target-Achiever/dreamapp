...
$sql_count_follow = "SELECT COUNT(followID) as count_follow FROM `dream_user_following` WHERE following_id='".$user_id."'";
$count_post = $post->execute($sql_count_post);
$count_follow = $post->execute($sql_count_follow);
#End of COUNT
if(empty($count_post['count_post']) && empty($count_follow['count_post']))
{
...
if(!empty($getposts))
{
    ...
    {		
		...
		$result[$i]['post_comment_count'] =  count($comment_count_result['count']);
