...
$action =  $data["action"];
	...
	if($action == 1)
	{
	 $query		= "SELECT T2.* FROM dream_user_following as T1 INNER JOIN dream_users as T2 ON T1.user_id = T2.id WHERE T1.following_id = '".$friend_id."'";	
	
	}
	else{
		
	$query		= "SELECT T2.* FROM dream_user_following as T1 INNER JOIN dream_users as T2 ON T1.following_id = T2.id WHERE T1.user_id = '".$friend_id."'";	
